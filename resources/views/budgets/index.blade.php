...
<h1 class="title1 text-center my-4">{{ __('Lista de Tratamientos') }}</h1>
...
<div class="hidden sm:block max-w-6xl mx-auto bg-white rounded-xl p-3 text-gray-900 shadow-md">
    ...
    <div class="grid grid-cols-5 gap-4 items-center border-b border-gray-200 py-3 text-gray-800 hover:bg-gray-50 transition text-center">
        ...
        <div><a href="{{ route('budgets.show', $budget->id) }}" class="hover:text-cyan-600">Bs. {{ number_format($budget->total_amount, 2) }}</a></div>
        <div><a href="{{ route('budgets.show', $budget->id) }}" class="hover:text-cyan-600">{{ $budget->description }}</a></div>
...
<div class="sm:hidden max-w-6xl mx-auto flex flex-col gap-3">
    ...
    <div class="bg-white rounded-lg shadow-md p-4 flex flex-col gap-2 hover:shadow-lg transition">
        <div class="flex justify-between items-center">
            ...
            <div class="text-gray-500 text-sm">{{ $budget->budget }}</div>
        </div>
        <div class="text-gray-600">Costo: Bs. {{ number_format($budget->total_amount, 2) }}</div>
        <div class="text-gray-600">Detalles: {{ $budget->description }}</div>
