...
<div class="hidden sm:block max-w-6xl mx-auto bg-white rounded-xl p-3 text-gray-900 shadow-md">
    ...
    <div class="grid grid-cols-7 gap-4 items-center border-b border-gray-200 py-3 text-gray-800 hover:bg-gray-50 transition text-center">
        <div>
            <a href="{{ route('patient.show', $patient->id) }}" class="hover:text-cyan-600">
                {{ $patient->ci_patient ?? '-' }}
            </a>
        </div>
        <div>
        ...
        <div>{{ $patient->patient_contact }}</div>

        <div class="flex justify-center gap-2">
            @if($patient->patient_contact)
            <a href="https://wa.me/{{ $patient->patient_contact }}" target="_blank"
               class="inline-flex items-center px-3 py-1 rounded-full bg-green-500 text-white hover:bg-green-600 transition">
                <img src="{{ asset('assets/images/whatsapp.png') }}" class="h-3 w-3 mr-2">WhatsApp
            </a>
            @endif
        </div>

        <div class="flex justify-center gap-2">
            <a href="{{ route('treatment.newcreate', $patient->id) }}" class="botton2">
                {{ __('Presupuesto') }}
            </a>
        </div>
    ...
    <p class="text-gray-600 text-center py-4">
        {{ __('No se encontraron resultados para su búsqueda.') }}
    </p>
    @endforelse
...
<div class="sm:hidden max-w-6xl mx-auto flex flex-col gap-3">
    ...
    <div class="bg-white rounded-lg shadow-md p-4 flex flex-col gap-2 hover:shadow-lg transition">
        ...
        <div class="flex gap-2 mt-2">
            <a href="{{ route('patient.edit', $patient->id) }}" class="botton3 flex-1 text-center">
                {{ __('Editar') }}
            </a>
            @auth
                @if(Auth::user()->role === 'admin')
